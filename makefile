...
MACROS := VULKAN_HPP_NO_STRUCT_CONSTRUCTORS

# put your fcompiler features here, -f prefix is assumed
FEATS:=concepts-diagnostics-depth=2

# you put your generic flags here, - prefix is assumed
CFLAGS=O0 g3 std=c++2a
LFLAGS=
...
COMPILE_SHD=$(SC)
COMPILE_CPP=$(CC) -MMD -c\
	    $(patsubst %, -%,$(CFLAGS))\
	    $(patsubst %, -I%,$(PATH_INCS))\
	    $(patsubst %, -W%,$(WARNS))\
	    $(patsubst %, -f%,$(FEATS))\
	    $(patsubst %, -D%, $(MACROS))
LINK_CPP=$(LD) -o $@ $^\
	 $(patsubst %,-rpath %,$(PATH_DLIB))\
	 $(patsubst %, -L%, $(PATH_LIBS))\
	 $(patsubst %, -l%, $(LIBS))\
	 $(patsubst %, -%, $(LFLAGS))
